...
from ckanext.features.interfaces import IFeature
...
class FeatureController(base.BaseController):
    """
    ...
    def _submit(context):
        ...
        if len(errors) == 0:
            ...
            mail_dict = {
                ...
                'recipient_name': config.get("ckanext.features.recipient_name", config.get('ckan.site_title')),
                'subject': config.get("ckanext.features.subject", 'Feature Request from visitor'),
                'body': body,
                'headers': {'reply-to': data_dict["email"]}
            }

            # Allow other plugins to modify the mail_dict
            for plugin in p.PluginImplementations(IFeature):
    ...
    def form(self):
        ...
        if data.get('success', False):
            return p.toolkit.render('success.html')
        else:
            vars = {'data': data, 'errors': errors, 'error_summary': error_summary}
            return p.toolkit.render('form.html', extra_vars=vars)
