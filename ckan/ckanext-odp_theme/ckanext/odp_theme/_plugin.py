...
class ODPThemePlugin(ODPSearchPlugin):
    ...
    def dataset_facets(self, facets_dict, package_type):
        ...
        if 'groups' in new_facets_dict:
            new_facets_dict['groups'] = tk._('Group')
        if 'Organizations' in new_facets_dict:
            new_facets_dict['Organizations'] = tk._('Organization')
        if 'Domains' in new_facets_dict:
            new_facets_dict['Domains'] = tk._('Domain')
        
        return new_facets_dict

    def group_facets(self, facets_dict, group_type, package_type):
        if 'groups' in facets_dict:
            facets_dict['groups'] = tk._('Group')
        if 'Organizations' in facets_dict:
            facets_dict['Organizations'] = tk._('Organizations')
        if 'Domains' in facets_dict:
            facets_dict['Domains'] = tk._('Domains')
    ...
    def organization_facets(self, facets_dict, organization_type,
        ...
        if 'groups' in facets_dict:
            facets_dict['groups'] = tk._('Group')
        if 'Organizations' in facets_dict:
            facets_dict['Organizations'] = tk._('Organization')
        return facets_dict

    def domain_facets(self, facets_dict, domain_type,
                            package_type):
        if 'groups' in facets_dict:
            facets_dict['groups'] = tk._('Group')
        if 'Domains' in facets_dict:
            facets_dict['Domains'] = tk._('Domain')
        return facets_dict
