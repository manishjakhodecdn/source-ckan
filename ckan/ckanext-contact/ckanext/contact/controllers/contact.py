...
class ContactController(base.BaseController):
    ...
    def _submit(context):
        ...
        if data_dict["contact_type"] == '':
            errors['contact_type'] = [u'Missing Value']
            error_summary['contact_type'] = u'Missing value'
        
        if len(errors) == 0:

            body = '%s' % data_dict["content"]
            body += '\n\nSent by:\nName:%s\nEmail: %s\nProvisioning Services: %s\n' % (data_dict["name"], data_dict["email"],data_dict['contact_type'])
            
            if data_dict['contact_type'] == 'data-provisioning' :
                email_to_service =   config.get('data_provisioning')
            else :
                email_to_service =   config.get('analytics_provisioning')
                        
            mail_dict = {
                'recipient_email': email_to_service,
