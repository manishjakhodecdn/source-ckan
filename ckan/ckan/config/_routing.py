...
def make_map():
    ...
    with SubMapper(map, controller='api', path_prefix='/api{ver:/1|/2|}',
        ...
        m.connect('/util/organization/autocomplete', action='organization_autocomplete',
                  conditions=GET)
        m.connect('/util/domain/autocomplete', action='domain_autocomplete',
                  conditions=GET)
    ...
    # domain these basically end up being the same as groups
    with SubMapper(map, controller='domain') as m:
        m.connect('domains_index', '/domain', action='index')
        m.connect('/domain/list', action='list')
        m.connect('/domain/new', action='new')
        m.connect('/domain/{action}/{id}',
                  requirements=dict(action='|'.join([
                      'delete',
                      'admins',
                      'member_new',
        ...
        m.connect('domain_activity', '/domain/activity/{id}',
                  action='activity', ckan_icon='time')
        m.connect('domain_read', '/domain/{id}', action='read')
        m.connect('domain_about', '/domain/about/{id}',
                  action='about', ckan_icon='info-sign')
        m.connect('domain_read', '/domain/{id}', action='read',
                  ckan_icon='sitemap')
        m.connect('domain_edit', '/domain/edit/{id}',
                  action='edit', ckan_icon='edit')
        m.connect('domain_members', '/domain/members/{id}',
                  action='members', ckan_icon='group')
        m.connect('domain_bulk_process',
                  '/domain/bulk_process/{id}',
                  action='bulk_process', ckan_icon='sitemap')
    lib_plugins.register_package_plugins(map)
    lib_plugins.register_group_plugins(map)
    ...
    with SubMapper(map, controller='user') as m:
        ...
        m.connect('user_dashboard_domains', '/dashboard/domains',
                  action='dashboard_domains', ckan_icon='building')
    ...
    with SubMapper(map, controller='feed') as m:
        ...
        m.connect('/feeds/domain/{id}.atom', action='domain')
